...
```{r, message=FALSE, warning=FALSE}
# install the 'weightr' package (if it is not already installed) and load it
...
res <- weightfunct(dat2$yi, dat2$vi, steps=c(0.05, 0.10, 0.50, 1.00))
res
```
...
si <- c(0, res$steps)
di <- c(1, res$output_adj$par[-c(1:2)])
di <- di / max(di)
tmp <- list(breaks=si, density=di, equidist=FALSE, xname="p-Value")
class(tmp) <- "histogram"
par(mar=c(5,4,2,2))
plot(tmp, ylab="Probability of Selection", main="", las=1, xaxt="n")
axis(side=1, at=si)
```
...
res.weak.1 <- weightfunct(dat2$yi, dat2$vi, steps=ssp$steps, weights=ssp$weak.1)
res.stro.1 <- weightfunct(dat2$yi, dat2$vi, steps=ssp$steps, weights=ssp$stro.1)
res.weak.2 <- weightfunct(dat2$yi, dat2$vi, steps=ssp$steps, weights=ssp$weak.2)
res.stro.2 <- weightfunct(dat2$yi, dat2$vi, steps=ssp$steps, weights=ssp$stro.2)

# Table 9.4 (but transposed)

tab <- data.frame(logOR = c(coef(res.un), res.weak.1$adj_est[2,], res.stro.1$adj_est[2,],
                                          res.weak.2$adj_est[2,], res.stro.1$adj_est[2,]),
                  varcomp = c(res.un$tau2, res.weak.1$adj_est[1,], res.stro.1$adj_est[1,],
                                           res.weak.2$adj_est[1,], res.stro.1$adj_est[1,]))
...
```{r, fig.width=9, fig.height=7, dev.args=list(pointsize=pointsize), fig.align='center'}
...
res <- with(dat1, weightfunct(yi, vi, mods = ~ weeks, fe=TRUE, steps=c(0.05, 0.30, 1.00)))
res
```
...
res.weak.1 <- with(dat1, weightfunct(yi, vi, mods = ~ weeks, fe=TRUE,
                                     steps=ssp$steps, weights=ssp$weak.1))
res.stro.1 <- with(dat1, weightfunct(yi, vi, mods = ~ weeks, fe=TRUE,
                                     steps=ssp$steps, weights=ssp$stro.1))
res.weak.2 <- with(dat1, weightfunct(yi, vi, mods = ~ weeks, fe=TRUE,
                                     steps=ssp$steps, weights=ssp$weak.2))
res.stro.2 <- with(dat1, weightfunct(yi, vi, mods = ~ weeks, fe=TRUE,
                                     steps=ssp$steps, weights=ssp$stro.2))

# Table 9.10 (but transposed)

tab <- data.frame(rbind(coef(res.un), res.weak.1$adj_est[1:2,], res.stro.1$adj_est[1:2,],
                                      res.weak.2$adj_est[1:2,], res.stro.2$adj_est[1:2,]))
rownames(tab) <- c("unadjusted", "weak one-tailed selection", "strong one-tailed selection",
                                 "weak two-tailed selection", "strong two-tailed selection")
dfround(tab, 2)
```

As far as I can tell, the `copas()` function from the [metasens](https://cran.r-project.org/package=metasens) package cannot handle meta-regression models. Hence, this part cannot be reproduced.
...
```{r}
# fit fixed-effects model to dataset 2 and obtain the estimated odds ratio (with 95% CI)

res <- rma(yi, vi, data=dat2, method="FE", slab=paste0(author, ", ", year))
...
```{r}
# conduct trim and fill analysis for dataset 2 and obtain the estimated odds ratio (with 95% CI)
...
funnel(rtf, xlim=c(-2,2), ylim=c(0,.8), las=1, digits=1,
...
sav <- cumul(res, order=order(dat2$vi))
