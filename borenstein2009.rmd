...
agg <- aggregate(dat, cluster=dat$study, struct="ID", select=c("study","es","var"))
agg
```
...
dat <- escalc(yi=es, vi=var, data=dat, var.names=c("es","var"))
agg <- aggregate(dat, cluster=dat$study, struct="CS", select=c("study","es","var"),
...
# the coefficient for 'outcomeReading' is the estimate of the difference
...
fsn(dat$yi, dat$vi, type="Orwin", target=log(1.05), weighted=TRUE)
```
