...
This document provides the R code to reproduce all examples and illustrations from the book *Introduction to Meta-Analysis* by Borenstein et al. (2009). The analyses are reproduced using the `metafor` package. To read more about the package, see the [package website](http://www.metafor-project.org/) and the [package documentation](https://wviechtb.github.io/metafor/). Note that the 'devel' version of `metafor` needs to be installed as some of the datasets used below are not currently in the official release of the package on [CRAN](https://cran.r-project.org/package=metafor). The 'devel' version of the package can be installed with:
...
2. Ocasionally, there are some minor discrepancies between the results shown in the book and those obtained below. Where such discrepancies arise, they are noted (and the reasons for them).
...
# note: the sampling variance is again computed in a slightly different way
# in the book compared to the metafor package; the difference is negligible
```
...
dat <- escalc("SMD", m1i=mean1, sd1i=sd1, n1i=n1, m2i=mean2, sd2i=sd2, n2i=n2, slab=study, data=dat)
dat

# note: the sampling variance of Hedges' g is computed in a slightly different
# way in the book compared to the metafor package; this leads to negligible
# differences between the results in the book and the results obtained here
```
...
1. The prediction interval is computed in a slightly different way in the book compared to what is shown below. For more details on how the prediction interval is computed by the `metafor` package, see [here](http://www.metafor-project.org/doku.php/faq#for_random-effects_models_fitt).
...
rtf <- trimfill(res, digits=3)
rtf
predict(rtf, transf=exp)
```
...
```{r}
### naive pooling by summing up the counts and then computing the odds ratio
### and corresponding confidence interval based on the aggregated table
...
```{r}
### load/install the poolr package if it is not already installed

if (suppressWarnings(!require(poolr, quietly=TRUE))) {
   install.packages("poolr", quiet=TRUE)
   library(poolr)
}
```
